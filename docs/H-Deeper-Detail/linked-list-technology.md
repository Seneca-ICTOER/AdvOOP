...
A *stack* is a special kind of linked list in which each node adds to the head and removes from the head.  In a stack, the last element in is the first element out (LIFO).
